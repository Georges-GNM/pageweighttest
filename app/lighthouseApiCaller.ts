//will need to add pagetype and title
export async function runLighthouse(articleUrl: string) {
    ...
    fetch(url)
      ...
      .then(json => {
        ...
        //   'weight': lighthouse.audits['total-byte-weight'].displayValue,
          time: lighthouse.fetchTime,
          'weight': lighthouse.audits['total-byte-weight'].displayValue,
          url: {articleUrl},
          title: lighthouse.audits.title,
          pageLoadTime: lighthouse.timing.total, //maybe not the best metric
          pageType: "Article",
        };
